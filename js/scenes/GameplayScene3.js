function drawStarPath(ctx, cx, cy, outerRadius, innerRadius, points = 5) {
  ...
  setBounds(canvasWidth, canvasHeight) {
    ...
    this.leftMinX = -120;
    this.leftMaxX = canvasWidth * 0.34;

    this.rightMinX = canvasWidth * 0.66;
    this.rightMaxX = canvasWidth + 120;

    this.minY = canvasHeight * 0.3;
  ...
  update(delta = 0.016) {
    ...
    const horizontalWave = (Math.sin(t * 0.95) + 1) * 0.5;
    const verticalWave = (Math.cos(t * 1.15) + 1) * 0.5;
  ...
  this.homeStars = [
  new HomeStar(this.canvas.width, this.canvas.height, "left", 0),
  new HomeStar(this.canvas.width, this.canvas.height, "right", Math.PI),
];
