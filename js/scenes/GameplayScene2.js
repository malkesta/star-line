function drawStarPath(ctx, cx, cy, outerRadius, innerRadius, points = 5) {
  ...
  draw(ctx) {
    ...
    const glowRadius = this.glowRadius * scale;

    const glowPulse = 0.92 + Math.sin(this.flicker) * 0.05;

    const outerGlow = ctx.createRadialGradient(this.x, this.y, 10, this.x, this.y, glowRadius);
    ...
    ctx.beginPath();
    ctx.arc(this.x, this.y, ringRadius - 6, 0, Math.PI * 2);
    ctx.lineWidth = 0.85;
    ...
    drawStarPath(ctx, this.x, this.y, radius, radius * 0.48, 5);
    const core = ctx.createRadialGradient(this.x - 8, this.y - 10, 4, this.x, this.y, radius);
    ...
    drawStarPath(ctx, this.x - 4, this.y - 6, radius * 0.35, radius * 0.15, 5);
  ...
  isHit(starlet) {
    const dx = starlet.x - this.x;
    const dy = starlet.y - this.y;
    const currentRadius = this.radius * (1 + Math.sin(this.flicker * 0.9) * 0.5);
    return Math.sqrt(dx * dx + dy * dy) < currentRadius + starlet.radius;
  }

  blocksObstacle(obstacle) {
    const dx = obstacle.x - this.x;
    const dy = obstacle.y - this.y;
    const currentRingRadius = this.ringRadius * (1 + Math.sin(this.flicker * 0.9) * 0.5);
    const dist = Math.sqrt(dx * dx + dy * dy);
    return dist < currentRingRadius + obstacle.ringRadius;
  }
  ...
  repelObstacle(obstacle) {
    ...
    const overlap = currentRingRadius + obstacle.ringRadius - dist;
